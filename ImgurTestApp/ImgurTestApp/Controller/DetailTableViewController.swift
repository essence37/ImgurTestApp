...
class DetailTableViewController: UITableViewController {
    ...
    var realmDetailItem: RealmGalleryAlbum? {
        didSet {
            if isViewLoaded {
                configureViewFromRealm()
            }
        }
    }
    ...
    func configureViewFromRealm() {
        guard let realmDetailItem = realmDetailItem else { return }
        
        containerView.frame.size.height = setViewHeightFromRealm(item: realmDetailItem)
        
        dateLabel.isHidden = false
        
        if let image = realmDetailItem.images.first {
            if image.link.contains(".mp4") {
                guard let videoURL = URL(string: image.link) else { return }
                playVideo(videoURL)
            }
            ...
            if realmDetailItem.link.contains(".mp4") {
                guard let videoURL = URL(string: realmDetailItem.link) else { return }
                playVideo(videoURL)
            }
            imageView.kf.setImage(with: URL(string: realmDetailItem.link))
        }
        dateLabel.text = realmDetailItem.date
    }
    ...
    func setViewHeightFromRealm(item: RealmGalleryAlbum) -> CGFloat {
        let viewWide = UIScreen.main.bounds.width
        guard let imageHieght = item.coverHeight.value, let imageWide = item.coverWidth.value else {
            return viewWide
        }
        let viewHieght = CGFloat(imageHieght) / CGFloat(imageWide) * viewWide
        return viewHieght
